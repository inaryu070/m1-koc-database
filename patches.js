...
function drawMap(){const years=yearsForRange(),from=years[0],to=years.at(-1),data=rangeAgency(from,to).slice(0,40),{ctx,w,h}=cctx(mapCanvas),pad={l:40,r:18,t:18,b:34},pw=w-pad.l-pad.r,ph=h-pad.t-pad.b,maxM=Math.max(1,...data.map(x=>x.m1)),maxK=Math.max(1,...data.map(x=>x.koc));
  ctx.font='8px Arial';ctx.lineWidth=1;ctx.textBaseline='middle';for(let i=0;i<=4;i++){const xx=pad.l+pw*i/4,yy=pad.t+ph-ph*i/4;ctx.strokeStyle='#25262b';ctx.beginPath();ctx.moveTo(xx,pad.t);ctx.lineTo(xx,h-pad.b);ctx.stroke();ctx.beginPath();ctx.moveTo(pad.l,yy);ctx.lineTo(w-pad.r,yy);ctx.stroke();ctx.fillStyle='#62656c';ctx.textAlign='center';ctx.fillText(Math.round(maxM*(i/4)*(i/4)),xx,h-pad.b+12);ctx.textAlign='right';ctx.fillText(Math.round(maxK*(i/4)*(i/4)),pad.l-6,yy)}ctx.textAlign='center';ctx.fillStyle='#74777e';ctx.fillText('M-1 POWER →',pad.l+pw/2,h-9);ctx.save();ctx.translate(10,pad.t+ph/2);ctx.rotate(-Math.PI/2);ctx.fillText('KOC POWER →',0,0);ctx.restore();
  mapHits=[];const labels=new Set(data.slice(0,10).map(x=>x.agency));for(const x of data){const cx=pad.l+Math.sqrt(x.m1/maxM)*pw,cy=pad.t+ph-Math.sqrt(x.koc/maxK)*ph,r=Math.min(13,4+Math.sqrt(x.n)*.65),ratio=x.power?x.m1/x.power:.5;ctx.fillStyle=ratio>=.6?'rgba(239,90,84,.58)':ratio<.4?'rgba(107,165,223,.58)':'rgba(223,188,104,.65)';ctx.strokeStyle=ratio>=.6?'rgba(239,150,146,.62)':ratio<.4?'rgba(155,195,235,.62)':'rgba(228,200,120,.7)';ctx.lineWidth=1;ctx.beginPath();ctx.arc(cx,cy,r,0,Math.PI*2);ctx.fill();ctx.stroke();mapHits.push({x:cx,y:cy,r:r+5,row:x});if(labels.has(x.agency)){ctx.fillStyle='#b9bbc0';ctx.font='8px Arial';ctx.textAlign='left';ctx.fillText(x.agency,cx+r+3,cy)}}}
function drawDNA(){const years=yearsForRange(),agency=agencyEl.value,series=agencyYearSeries(agency,years),{ctx,w,h}=cctx(dnaCanvas),pad={l:35,r:18,t:16,b:30},pw=w-pad.l-pad.r,ph=h-pad.t-pad.b,max=Math.max(1,...series.flatMap(x=>[x.m1,x.koc]));ctx.font='8px Arial';ctx.textBaseline='middle';for(let i=0;i<=4;i++){const yy=pad.t+ph-ph*i/4;ctx.strokeStyle='#25262b';ctx.beginPath();ctx.moveTo(pad.l,yy);ctx.lineTo(w-pad.r,yy);ctx.stroke();ctx.fillStyle='#62656c';ctx.textAlign='right';ctx.fillText(Math.round(max*i/4),pad.l-6,yy)}const step=pw/Math.max(1,years.length-1);years.forEach((y,i)=>{if(i%Math.max(1,Math.ceil(years.length/7))===0||i===years.length-1){ctx.fillStyle='#62656c';ctx.textAlign='center';ctx.fillText(String(y),pad.l+i*step,h-pad.b+12)}});
